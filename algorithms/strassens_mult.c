...
int matrix_mult (int dim, int matrixarray1[dim][dim], int matrixarray2[dim][dim])
{
    ...
    for (row = 0; row < dim; row++){
        for (col = 0; col < dim; col++){
            matrixarray3[row][col] = matrixarray2[col][row];
        }
    }
    ...
    do
    {
        for (row = 0; row < dim; row++)
        {
            for (col = 0; col < dim; col++)
            {
                    int suma = 0;
                    for (l = 0; l < dim; l++)
                            suma += matrixarray1[row][l] * matrixarray3[col][l];
                    
                    // printf ("%d ", suma);
                    matrix_result[row][col] = suma;
            }
         }
    c++;
    } while (c < dim);
...
int matrix_add (int matrixarray1[dim][dim], int matrixarray2[dim][dim])
{
    int row,col;
    ...
    for (row = 0; row < dim; row++)
    {
        for (col = 0; col < dim; col++)
        {
                matrixarray1[row][col] = matrixarray1[row][col] + matrixarray2[row][col];
        }
     }
